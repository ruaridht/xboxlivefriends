...
#import "AIAchievementsController.h"
...
@implementation AIAchievementsController

- (NSString *)notificationName {
	return @"AIAchievementsLoadNotification";
}
...
- (void)displayAccountInfo:(NSString *)gamertag {
	...
	if (gamesPlayed) {
		...
		[[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:@"AIShowErrorTab" object:@"An Error Occurred"]];
	}
	...
	if([gamesList count] != 0){
		...
		for (XBGame *thisGame in gamesList){
			...
			@try{
				...
				int totalPointsPossible = [[self totalPointsFromString:[thisGame yourScore]] intValue];
...
- (void)webViewAchievementDetails:(NSDictionary *)info {
	NSLog(@"Viewing Achievement details");
	...
	NSArray *achievementArray = [XBAchievementDetailsParser fetchForSelfWithGameID:gameID];
	NSString *achievementDetailsSource = @"";
	NSString *rowSource = [NSString stringWithContentsOfFile:[NSString stringWithFormat:@"%@%@", [[NSBundle mainBundle] resourcePath], @"/achievement_row_self.htm"] encoding:NSMacOSRomanStringEncoding error:NULL];
	...
	for (XBAchievement *theAchievement in achievementArray){
		...
		if ([theAchievement iHaveAchievement]) {
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:@"AIStopProgressIndicator" object:nil];
