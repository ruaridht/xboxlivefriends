...
#include "AITabController.h"
#import "ClickableAttachedWindow.h"

#import "Xbox Live Friends.h"
#import "AIBreakdownChartController.h"
#import "XBGamesPlayedParser.h"
#import "ColorizeImage.h"

#define round(x) ((x)>=0?(long)((x)+0.5):(long)((x)-0.5))

@implementation AIBreakdownChartController
...
- (void)awakeFromNib {
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tabWillChange:) name:@"AccountInfoTabWillChange" object:nil];
}

- (NSString *)notificationName {
	return @"AIBreakdownChartLoadNotification";
}
...
- (void)displayPieChart:(NSArray *)gameList {
	...
	if([gameList count] != 0) {
		...
		for (XBGame *thisGame in gameList) {
			...
			NSString *score = [thisGame yourScore];
			/*
